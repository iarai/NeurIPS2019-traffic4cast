...
- the data is then split along the first axis in 48 samples of 6 time bins each (3 as input, 3 as ground truth)
...
def load_data(file_path):
    ...
    data = np.array(np.split(data, 48, axis=0), dtype=np.float32)
...
def model_train(data_dir, model_dir, log_path, excl_dates=[]):
